...
class BatchedGeneticAlgorithm:
    ...
    def calculate_fitness(self):
        """Calculates the fitness for every individual in every population, all at once."""
        with torch.no_grad():
            # Reshape for a single massive batch forward pass
            pop_reshaped = self.population.view(-1, self.latent_dim)
            
            # Get model output
            logits = self.model.get_logits_from_z(pop_reshaped)
            
            # Reshape logits back to (num_samples, pop_size, seq_len, vocab_size)
            logits = logits.view(self.num_samples, self.population_size, -1, VOCAB_SIZE)
            
            # Prepare targets by expanding them
            targets_expanded = self.target_grids.unsqueeze(1).expand(-1, self.population_size, -1)
            
            # 1. Reconstruction Loss (Cross-Entropy)
            # Reshape for loss function: combine samples and population into a single batch dimension
            ...
            ce_loss = nn.functional.cross_entropy(
                ...
                ce_targets,
                reduction='none'
            ).mean(dim=1).view(self.num_samples, -1) # Reshape back to (num_samples, pop_size)

            # 2. KL Divergence Penalty
            kld = 0.5 * torch.sum(self.population.pow(2), dim=2) # Shape: (num_samples, pop_size)

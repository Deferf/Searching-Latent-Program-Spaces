...
def vae_loss_function(logits, x, mu, log_var, input_data, mask_weight=1.0):
...
def train(model, dataloader, optimizer, device, mask_weight):
    model.train()
    total_loss = 0
    for input_data, output_data in dataloader:
        ...
        loss = vae_loss_function(logits, output_data, mu, log_var, input_data, mask_weight)
        loss.backward()
        optimizer.step()
...
def evaluate(model, dataloader, device):
    ...
    with torch.no_grad():
        for input_data, output_data in dataloader:
            ...
            loss = vae_loss_function(logits, output_data, mu, log_var, input_data, mask_weight=0.0) # Evaluate on full reconstruction
            total_loss += loss.item()
    return total_loss / len(dataloader.dataset)
...
def calculate_manhattan_distance(model, dataloader, device):
    ...
    with torch.no_grad():
        for input_data, output_data in dataloader:
            ...
            generated_outputs_flat = model.decode_from_z(mu).view(-1, 100)

            # De-quantize ground truth for comparison
            ground_truth_flat = output_data.float() / (VOCAB_SIZE - 1)
            ground_truth_flat = ground_truth_flat.to(device).view_as(generated_outputs_flat)

            # Calculate Manhattan distance
            total_distance += torch.abs(generated_outputs_flat - ground_truth_flat).sum().item()
...
def main(args):
    ...
    BATCH_SIZE = 8
    MODEL_SAVE_PATH = os.path.join(args.output_dir, 'best_categorical_vae.pth')
    DO_TRAIN = True
    
    # Create output directory if it doesn't exist
    os.makedirs(args.output_dir, exist_ok=True)
    
    print(f"Running experiment with loss threshold: {args.loss_threshold}")
    ...
    if DO_TRAIN:
        ...
        # --- Adaptive Decrement Hyperparameters ---
        ...
        print("Starting training with gradient-based adaptive weighted mask...")
        for epoch in range(1, MAX_EPOCHS + 1):
            train_loss = train(model, train_loader, optimizer, DEVICE, mask_weight)
            
            if not mask_reduction_enabled and train_loss < args.loss_threshold:
                print(f'  -> Train loss below {args.loss_threshold}. Mask weight reduction is now enabled.')
                mask_reduction_enabled = True

            # --- Validation Step ---
            val_manhattan_distance = calculate_manhattan_distance(model, val_loader, DEVICE)
            
            print(f'Epoch {epoch}/{MAX_EPOCHS}, Train Loss: {train_loss:.4f}, Validation Set Manhattan Distance: {val_manhattan_distance}, Mask Weight: {mask_weight:.3f}')

            if val_manhattan_distance < best_val_manhattan:
                best_val_manhattan = val_manhattan_distance
                epochs_since_improvement = 0
                torch.save(model.state_dict(), MODEL_SAVE_PATH)
                print(f'  -> New best Validation Set Manhattan Distance: {best_val_manhattan}. Model saved.')
            else:
            ...
            if mask_weight <= 1e-6:
                print(f'\nMask weight reached zero at epoch {epoch}. Stopping training.')
                break

            if mask_reduction_enabled and epochs_since_improvement >= patience:
                # Calculate the gradient magnitude (|∂L/∂w|)
                unmasked_loss = calculate_unmasked_loss(model, train_loader, DEVICE)
                
                # Calculate the dynamic decrement
                decrement_size = SCALING_FACTOR / (unmasked_loss + EPSILON)
                
                # Clamp the decrement to prevent extreme values
                decrement_size = max(MIN_DECREMENT, min(decrement_size, MAX_DECREMENT))

                print(f'  -> Validation Set Manhattan Distance did not improve for {patience} epochs.')
                print(f'  -> Unmasked Loss (Gradient Magnitude): {unmasked_loss:.4f}, Calculated Decrement: {decrement_size:.4f}')
                
                mask_weight = max(0.0, mask_weight - decrement_size)
                print(f'  -> Reducing mask weight to {mask_weight:.3f}.')

                epochs_since_improvement = 0 # Reset patience
                mask_reduction_enabled = False
                print(f'  -> Mask weight reduction is now disabled until train loss is below 0.001 again.')
        ...
        final_model_path = os.path.join(args.output_dir, 'final_vae_transformer.pth')
    ...
    visualize_results(best_model, val_loader, DEVICE, filename=os.path.join(args.output_dir, 'best_model_inference_results.png'))
    ...
    visualize_results(final_model, train_loader, DEVICE, filename=os.path.join(args.output_dir, 'final_model_train_inference_results.png'))
    print("\n--- Generating Visualization on Validation Set (Final Model) ---")
    visualize_results(final_model, val_loader, DEVICE, filename=os.path.join(args.output_dir, 'final_model_val_inference_results.png'))

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train a Transformer VAE for grid reconstruction.')
    parser.add_argument('--loss_threshold', type=float, default=0.1, help='Loss threshold to enable mask weight reduction.')
    parser.add_argument('--output_dir', type=str, default='.', help='Directory to save logs, checkpoints, and visualizations.')
